...
parser.add_argument('--img_size_max', default=140, type=int, help='Input image size')
parser.add_argument('--img_size_min', default=28, type=int, help='Input image size')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    Vnet3d = Vnet3dModule(96, 96, 16, channels=1, costname=("dice coefficient",), inference=True,
            model_path="/data/shanyx/larry/LUNA16-Lung-Nodule-Analysis-2016-Challenge/segmeation/model/Vnet3d.pd-50000")

    for stage in range(args.stage, args.num_scale + 1):
        if args.distributed:
            train_sampler.set_epoch(stage)

        trainSinGAN(Vnet3d, train_loader, networks, {"d_opt": d_opt, "g_opt": g_opt}, stage, args, {"z_rec": z_fix_list})
