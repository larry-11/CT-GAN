...
class Generator(nn.Module):
    def __init__(self, img_size_min, num_scale, scale_factor=4/3):
        ...
        first_generator.append(nn.Sequential(nn.Conv3d(1, self.nf, 3, 1),
                                             nn.BatchNorm3d(self.nf),
                                             nn.LeakyReLU(2e-1)))
        for _ in range(3):
            first_generator.append(nn.Sequential(nn.Conv3d(self.nf, self.nf, 3, 1),
                                                 nn.BatchNorm3d(self.nf),
                                                 nn.LeakyReLU(2e-1)))

        first_generator.append(nn.Sequential(nn.Conv3d(self.nf, 1, 3, 1),
                                             nn.Tanh()))
    ...
    def progress(self):
        ...
        tmp_generator.append(nn.Sequential(nn.Conv3d(1, self.nf, 3, 1),
                                           nn.BatchNorm3d(self.nf),
                                           nn.LeakyReLU(2e-1)))

        for _ in range(3):
            tmp_generator.append(nn.Sequential(nn.Conv3d(self.nf, self.nf, 3, 1),
                                               nn.BatchNorm3d(self.nf),
                                               nn.LeakyReLU(2e-1)))

        tmp_generator.append(nn.Sequential(nn.Conv3d(self.nf, 1, 3, 1),
                                           nn.Tanh()))
