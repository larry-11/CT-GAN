...
def savemhd(gen, inp, i,stage):
    inp = inp.mul(255).cpu().detach().numpy().squeeze(0).squeeze(0)
    gen = gen.mul(255).cpu().detach().numpy().squeeze(0).squeeze(0)
    outpath = '../data'
    cv2.imwrite("%s/stage%s_inp0_%s.png" % (outpath, str(stage), str(i)), inp[0,:,:])
    cv2.imwrite("%s/stage%s_inp1_%s.png" % (outpath, str(stage), str(i)), inp[:,0,:])
    cv2.imwrite("%s/stage%s_inp2_%s.png" % (outpath, str(stage), str(i)), inp[:,:,0])
    cv2.imwrite("%s/stage%s_gen0_%s.png" % (outpath, str(stage), str(i)), gen[0,:,:])
    cv2.imwrite("%s/stage%s_gen1_%s.png" % (outpath, str(stage), str(i)), gen[:,0,:])
    cv2.imwrite("%s/stage%s_gen2_%s.png" % (outpath, str(stage), str(i)), gen[:,:,0])
